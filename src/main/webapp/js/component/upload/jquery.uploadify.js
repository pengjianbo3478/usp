...
SWFUpload.completeURL = function(a) {
	/* modify by cp 20140527,解决控制台错误，如："NetworkError: 404 Not Found - http://localhost:8080/cms/tm/lecturer/"
	 * 原来的代码，修改为直接返回a（这里为了设置上传的url，上传的button的背景图片的url）
			...
			return this
					.each(function() {
						...
						var settings = $.extend({
							...
							//添加的代码 20140527，解决页面上传控件添加upload_url属性，页面上传组件初始化之后，会被覆盖掉的问题 
							//start
							//flash的swf文件的路径前缀,目前固定值为${ctx}
							data_swf_global_url : $this.attr('data-swf-global-url'),
							//文件上传后台接收URL
							data_url_upload : $this.attr('data-url-upload'),
							//文件删除后台接收URL
							data_url_del : $this.attr('data-url-del'),
							//文件预览后台访问URL，只定义到该文件的访问路径（除去最后的文件名）
							data_url_priview : $this.attr('data-url-priview'),
							//文件裁剪结果后台接收URL
							data_url_cut_result : $this.attr('data-url-cut-result'),
							//end
						...
						if (flashInstalled) {
							...
							var $wrapper = $('<div />', {
								...
								//添加的代码 20140527，解决页面上传控件添加upload_url属性，页面上传组件初始化之后，会被覆盖掉的问题 
								//start
								'data-swf-global-url' : settings.data_swf_global_url,
								'data-url-upload' : settings.data_url_upload,
								'data-url-del' : settings.data_url_del,
								'data-url-priview' : settings.data_url_priview,
								'data-url-cut-result' : settings.data_url_cut_result,
								//end
								'class' : 'uploadify',
								'css' : {
...
//			this.queueData.errorMsg = 'Some files were not added to the queue:';
			this.queueData.errorMsg = '文件没有被添加到队列中:';
...
//								+ errorMsg + ').';
						...
						this.queueData.errorMsg += '\n上传的文件数量已经超出系统限制的['+this.settings.queueSizeLimit+']个文件！';
					}
					break;
...
//				alert('The upload limit has been reached (' + errorMsg + ').');
		...
		onUploadProgress : function(file, fileBytesLoaded, fileTotalBytes) {
			...
			if ($.inArray('onUploadProgress', settings.overrideEvents) < 0) {
				...
				*/
				//修改后的代码start  20140523
				$('#' + file.id).find('.data').html(' - ' + this.queueData.averageSpeed + suffix+"   - "+ percentage + '%');
				//修改后的代码end 	 20140523
				$('#' + file.id).find('.uploadify-progress-bar').css('width',
						percentage + '%');
			}
